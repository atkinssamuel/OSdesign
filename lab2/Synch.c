  ...
  behavior Mutex
    ...
    --         on the mutex.
	  ...
	  if self.IsHeldByCurrentThread ()
	    FatalError("Cannot lock: Mutex already owned")
	  endIf
	  
	  while !self.IsHeldByCurrentThread ()
            if heldBy != null -- Mutex already held by another thread, put thread to sleep
      ...
      method Unlock ()
          var
            ...
            t: ptr to Thread
          oldIntStat = SetInterruptsTo (DISABLED)
	  if !self.IsHeldByCurrentThread() -- You can't unlock the mutex if you don't own it
	    FatalError("Cannot unlock: Mutex not owned")
	  endIf
	  if !waitingThreads.IsEmpty () -- If there are threads waiting, wake one up
	    t = waitingThreads.Remove ()
            t.status = READY
            readyList.AddToEnd (t)   
	  endIf
          heldBy = null -- Release the mutex
	  oldIntStat = SetInterruptsTo (oldIntStat)  
        endMethod
