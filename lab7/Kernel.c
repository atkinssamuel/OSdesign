...
-----------------------------  InitializeScheduler  ---------------------------------
  function InitFirstProcess ()
    ...
    (*thPt).Fork(StartUserProcess, "TestProgram4" asInteger)
  endFunction
  ...
  function StartUserProcess (arg: int)
   ...
   filePt = fileManager.Open(arg asPtrTo array[*] of char)
    ...
    functionPlaceholder = SetInterruptsTo(DISABLED)
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
	...
	functionPlaceholder = SetInterruptsTo (ENABLED)
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      var
        ...
        functionPlaceholder: int
      functionPlaceholder = SetInterruptsTo(DISABLED)
      functionPlaceholder = (*currentThread).myProcess.addrSpace.GetStringFromVirtual(&fileName, filename asInteger, MAX_STRING_SIZE)
      if functionPlaceholder < 0
        return - 1
      endIf
      newAddrSpace.Init()
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
    ...
    fDI = -1
    functionPlaceholder = currentThread.myProcess.addrSpace.GetStringFromVirtual(&stringBuffer, filename asInteger, MAX_STRING_SIZE)


    if fileManager.openFileFreeList.IsEmpty() == false
      if (StrEqual(& strBuffer, "terminal"))
        openFilePointer = &fileManager.serialTerminalFile
      else
        openFilePointer = fileManager.Open(& strBuffer)
      endIf
    endIf

    if openFilePointer != null && functionPlaceholder != - 1
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
    ...
    if fileDesc < 0 || fileDesc > MAX_FILES_PER_PROCESS-1 || sizeInBytes < 0 || currentThread.myProcess.fileDescriptor[fileDesc] == null
      return -1
    endIf
          ...
          while true
            ...
            switch * DISK_STATUS_WORD_ADDRESS
              ...
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_1:
                FatalError ("Disk j/O error in SynchReadSector: Memory addr is not page-aligned or sector count is not positive")
          ...
          while true
            ...
            switch * DISK_STATUS_WORD_ADDRESS
              case DISK_STATUS_OPERATION_COMPLETED_OK:
                ...
                return
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_1:
                FatalError ("Disk j/O error in SynchWriteSector: Memory addr is not page-aligned or sector count is not positive")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_2:
