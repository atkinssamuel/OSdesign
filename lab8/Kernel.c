...
-----------------------------  InitializeScheduler  ---------------------------------
  function InitFirstProcess ()
    var thPt: ptr to Thread
    -- Create a new thread object by invoking GetANewThread
    thPt = threadManager.GetANewThread()
    -- Initialize the thread, name it something like "UserProgram"
    (*thPt).Init("UserProgram")
    -- Invoke Fork to start it running, supply zero as an argument
    (*thPt).Fork(StartUserProcess, "TestProgram5" asInteger)
  endFunction
------------------------------ StartUserProcess ------------------------------------
  function StartUserProcess (arg: int)
    var
      PCB: ptr to ProcessControlBlock
      filePt: ptr to OpenFile
      initPC: int
      initUserStackTop: int
      initSystemStackTop: int
      res: int
    -- First thing: allocate a new PCB
    PCB = processManager.GetANewProcess()
    -- Connect the PCB with the thread
    PCB.myThread = currentThread
    PCB.Init()

    -- Open the executable file and "hardcode" the filename with "TestProgram5"
   currentThread.myProcess = PCB

   filePt = fileManager.Open(arg asPtrTo array[*] of char)
   if filePt == null
    printInt(1)
   endIf
    -- Create the virtual address space and read the executables into it
    initPC = filePt.LoadExecutable(&(PCB.addrSpace))
    -- Close the executable file
    fileManager.Close(filePt)
    -- Compute init value for user-level stack
...
----------------------------- Serial Handler -----------------------------------
	...
	function SerialHandlerFunction(functionPlaceholder: int)
...
----------------------------- Serial Driver -----------------------------------
	...
	behavior SerialDriver
			method Init()
				...
				newTh.Init("serial-handler-thread")
				newTh.status = JUST_CREATED
				newTh.Fork(SerialHandlerFunction, 0)
			...
			method SerialHandler()
				var
					integerCharacter: char
					statusPlaceholder: int
        ...
        -- Wait for the serial to be initialized
				...
				while true
					...
					statusPlaceholder = *(serial_status_word_address)
					if (statusPlaceholder & SERIAL_CHARACTER_AVAILABLE_BIT) > 0
						integerCharacter = intToChar(*(serial_data_word_address))

						if getBufferSize == SERIAL_GET_BUFFER_SIZE
							print("\nSerial input buffer overrun - character '")
							printChar(integerCharacter)
							print("' was ingored\n")

						else
							serialLock.Lock()
							getBuffer[getBufferNextIn] = integerCharacter
				...
				if (statusPlaceholder & SERIAL_OUTPUT_READY_BIT) > 0
					serialLock.Lock()
					if putBufferSize > 0
						integerCharacter = putBuffer[putBufferNextOut]
						putBufferNextOut = (putBufferNextOut + 1) % SERIAL_PUT_BUFFER_SIZE
						putBufferSize = putBufferSize - 1
						*(serial_data_word_address) = charToInt(integerCharacter)
						putBufferSem.Up()
					endIf
					serialLock.Unlock()
				endIf
      ...
      method GetChar() returns char
        ...
        integerCharacter = getBuffer[getBufferNextOut]
        getBufferSize = getBufferSize-1
...
-----------------------------  Semaphore  ---------------------------------
  ...
  behavior Semaphore
    ...
    --         Up().  If initialized with j, then it is as if j Up()
...
-----------------------------  Condition  ---------------------------------
  ...
  behavior Condition
    ...
    -- have snuck in and run.  The waiting thread should always re-res the
  ...
  behavior Thread
      ...
      method Print ()
        ...
        --
          ...
          print ("  Thread \"")
          print (name)
          print ("\"    (destAddr of Thread object: ")
          ...
          printHexVar ("    stack starting destAddr", (& systemStack[0]) asInteger)
          switch status
            case JUST_CREATED:
  ...
  behavior ThreadManager
	...
	var
	  ...
	  for j = 0 to MAX_NUMBER_OF_PROCESSES - 1
		...
		threadTable[j].regs = new array of int {13 of 0}
	  	freeList.AddToFront(&threadTable[j])
          endFor
      ...
      method Print ()
        ...
        var j, oldStatus: int
          ...
          for j = 0 to MAX_NUMBER_OF_PROCESSES-1
            ...
            printInt (j)
            print (":")
            ThreadPrintShort (&threadTable[j])
          endFor
...
/*
          ...
          for j = 0 to MAX_FILES_PER_PROCESS-1
            if fileDescriptor[j]
              fileDescriptor[j].Print ()
            endIf
          endFor
*/
          nl ()
      ...
      method PrintShort ()
        ...
        --
          print ("  ProcessControlBlock   (destAddr=")
          printHex (self asInteger)
          print (")   pid=")
          printInt (pid)
          print (", status=")
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
	...
	for j = 0 to MAX_NUMBER_OF_PROCESSES - 1
	  processTable[j].status = FREE
	  processTable[j].addrSpace = new AddrSpace
	  processTable[j].addrSpace.Init()
	  freeList.AddToFront(&processTable[j])
	endFor
        endMethod
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        var item: ptr to ProcessControlBlock
	--
    ...
    item.Init()
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
        --
        FatalError("should never have been called")
        endMethod
	...
	-- First, lock the processManager since we are messing with other PCBs
	processManager.processManagerLock.Lock()

	for j = 0 to MAX_NUMBER_OF_PROCESSES - 1
		-- Identify parent
		if processManager.processTable[j].pid == p.parentsPid
			parentPcb = &(processManager.processTable[j])
		endIf
	endFor
	for j = 0 to MAX_NUMBER_OF_PROCESSES - 1
		-- Identify all children of this process who are zombies
		if processManager.processTable[j].status == ZOMBIE && processManager.processTable[j].parentsPid == p.pid
			-- Change its status to FREE and add it back to the PCB free list
			processManager.processTable[j].status = FREE
			processManager.freeList.AddToEnd(&processManager.processTable[j])
			-- Signal the aProcessBecameFree condition variable, since other threads may be
			-- waiting for free PCBs
			processManager.aProcessBecameFree.Signal(&(processManager.processManagerLock))
		endIf
	endFor
	if parentPcb != null && parentPcb.status == ACTIVE
		-- Turn p into a zombie
		p.status = ZOMBIE
...
-----------------------------  ProcessFinish  --------------------------
  ...
  function ProcessFinish (exitStatus: int)
    ...
    -- current thread.
    --
    -- Function is called by the process' thread. It will free all resources held by this
    -- process and will terminate the current thread. The PCB will be turned into a zombie
    ...
    for j = 0 to MAX_FILES_PER_PROCESS - 1
        if currentThread.myProcess.fileDescriptor[j] != null
            fileManager.Close(PCB.fileDescriptor[j])
            PCB.fileDescriptor[j] = null
        endIf
    endFor

    -- Set isUserThread to false
    currentThread.isUserThread = false

    -- Enable interrupts
    oldStatus = SetInterruptsTo (oldStatus)

    -- Return all page frames and invoke TurnIntoZombie
  ...
  behavior FrameManager
      ...
      method Init ()
        ...
        var j: int
          ...
          -- res that the area to be used for paging contains zeros.
          ...
          for j = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME
	  ...
	  for j = 0 to numFramesNeeded - 1
	    ...
	    numberFreeFrames = numberFreeFrames - 1
	    frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (f * PAGE_SIZE)
	    aPageTable.SetFrameAddr(j, frameAddr)
	  ...
	  for j = 0 to numFramesReturned - 1
	    frameAddr = aPageTable.ExtractFrameAddr(j)
...
-----------------------------  AddrSpace  ---------------------------------
  ...
  behavior AddrSpace
      ...
      method Print ()
        ...
        --
          var j: int
          print ("        destAddr        entry          Virtual    Physical   Undefined Bits  Dirty  Referenced  Writeable  Valid\n")
          print ("     ==========   ==========     ==========  ==========  ==============  =====  ==========  =========  =====\n")
          for j = 0 to numberOfPages-1
            print ("     ")
            printHex ((&pageTable[j]) asInteger)
            print (":  ")
            printHex (pageTable[j])
            print ("     ")
            printHex (j * PAGE_SIZE)   -- Virtual address
            print ("  ")
            printHex (self.ExtractFrameAddr (j))       -- Physical address
            print ("    ")
            if self.ExtractUndefinedBits (j) != 0
              printHex (self.ExtractUndefinedBits (j))
            else
              print ("          ")
            endIf
            print ("     ")
            ...
            endIf
            print ("      ")
            if self.IsReferenced (j)
              print ("YES")
            else
              print ("   ")
            endIf
            print ("         ")
            if self.IsWritable (j)
              print ("YES")
            else
              print ("   ")
            endIf
      ...
      method CopyBytesFromVirtual (kernelAddr, virtAddr, numBytes: int)
        ...
        --
          var curCopied, virtPage, offset, fromAddr: int
          ...
          while true
            ...
            -- printHexVar ("  Copying bytes from physcial destAddr", fromAddr)
            while offset < PAGE_SIZE
              -- printHexVar ("  Copying a byte to physcial destAddr", kernelAddr)
              -- printChar (* (fromAddr asPtrTo char))
              * (kernelAddr asPtrTo char) = * (fromAddr asPtrTo char)
              offset = offset + 1
              kernelAddr = kernelAddr + 1
              fromAddr = fromAddr + 1
              curCopied = curCopied + 1
              if curCopied == numBytes
                return curCopied
              endIf
            endWhile
            virtPage = virtPage + 1
      ...
      method CopyBytesToVirtual (virtAddr, kernelAddr, numBytes: int)
        ...
        -- addressed data, it returns -1.
        --
          var curCopied, virtPage, offset, destAddr: int
          if numBytes == 0
            return 0
          elseIf numBytes < 0
            return -1
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
	...
	var
		...
		oldPC: int
		newTh: ptr to Thread
		newPC: ptr to ProcessControlBlock
		res: int
	-- Allocate and set up new Thread and ProcessControlBlock objects
	newTh = threadManager.GetANewThread()
	newPC = processManager.GetANewProcess()
	newTh.Init("0")
	newPC.myThread = newTh
	newPC.parentsPid = currentThread.myProcess.pid
	newTh.myProcess = newPC
	newTh.status = READY

	-- Grab the values in the user registers and store a copy of them in the newTh
	-- object, use SaveUserRegs
	SaveUserRegs(&(newTh.userRegs[0]))
	res = SetInterruptsTo (ENABLED)

  for j = 0 to MAX_FILES_PER_PROCESS - 1
      if currentThread.myProcess.fileDescriptor[j] != null
          currentThread.myProcess.fileDescriptor[j].numberOfUsers = currentThread.myProcess.fileDescriptor[j].numberOfUsers + 1
            newTh.myProcess.fileDescriptor[j] = currentThread.myProcess.fileDescriptor[j]
       endIf
  endFor

	-- Initialize the stackTop pointer
	newTh.stackTop = &(newTh.systemStack[SYSTEM_STACK_SIZE - 1])

	-- Make a copy of the parent’s virtual address space. You will need to see how
	-- many pages are in the parent’s address space and call frameManager.GetNewFrames
	frameManager.GetNewFrames(&(newPC.addrSpace), currentThread.myProcess.addrSpace.numberOfPages)

	-- Run through each page index and copy the page. You can use MemoryCopy to do this efficiently
	for j = 0 to (currentThread.myProcess.addrSpace.numberOfPages-1)
		MemoryCopy(newPC.addrSpace.ExtractFrameAddr(j) , currentThread.myProcess.addrSpace.ExtractFrameAddr(j), PAGE_SIZE)
		if currentThread.myProcess.addrSpace.IsWritable(j) == true
			newPC.addrSpace.SetWritable(j)
		else
			newPC.addrSpace.ClearWritable(j)
		endIf
	endFor

	-- Call GetOldUserPCFromSystemStack () returns int
	oldPC = GetOldUserPCFromSystemStack()

	-- Invoke Fork
	newTh.Fork(ResumeChildAfterFork, oldPC)
	return newPC.pid
  endFunction
	...
	function ResumeChildAfterFork(PC: int)
	    ...
	    --Toggle interrupts
	    res = SetInterruptsTo(DISABLED)
	    --Mimic code in StartUserProcess
	    currentThread.myProcess.addrSpace.SetToThisPageTable()
	    RestoreUserRegs(&(currentThread.userRegs[0]))
	    currentThread.isUserThread = true
	    ...
	    BecomeUserThread(initUserStackTop, PC, initSystemStackTop)
	endFunction
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      -- NOT IMPLEMENTED
		...
		exitStatus: int
	    for j = 0 to MAX_NUMBER_OF_PROCESSES - 1
		-- Identify the child process and make sure that the pid that is passed in is the pid of
		-- a valid process and that is truly a child of this process
		if processManager.processTable[j].parentsPid == currentThread.myProcess.pid && processManager.processTable[j].pid == processID && processManager.processTable[j].status != FREE
		   exitStatus = processManager.WaitForZombie(&(processManager.processTable[j]))
		   return exitStatus
		endIf
	    endFor
	    -- If it isn't valid, return -1
	    return -1
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      var
        ...
        fileName: array [MAX_STRING_SIZE] of char
        initUserStackTop: int
        initSystemStackTop: int
        initPC: int
        filePt: ptr to OpenFile
        res: int
      res = SetInterruptsTo(DISABLED)
      res = (*currentThread).myProcess.addrSpace.GetStringFromVirtual(&fileName, filename asInteger, MAX_STRING_SIZE)
      if res < 0
        return - 1
      endIf
      newAddrSpace.Init()
      filePt = fileManager.Open(&fileName)
      if filePt == null
        return - 1
      endIf
      initPC = filePt.LoadExecutable(&newAddrSpace)
      if initPC < 0
        return - 1
      endIf
      frameManager.ReturnAllFrames(&((*currentThread).myProcess.addrSpace))
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      -- NOT IMPLEMENTED
            ...
            nl()
            print("virt destAddr of filename = ")
    ...
    -----------------------------  Handle_Sys_Open  ---------------------------------
      ...
      function Handle_Sys_Open (filename: ptr to array of char) returns int
        ...
        res = (*currentThread).myProcess.addrSpace.GetStringFromVirtual(&stringBuffer, filename asInteger, MAX_STRING_SIZE)

        -- Check empty slots and then return
        fDI = -1
        ...
        if (fDI != -1)
          ...
          if (StrEqual(& stringBuffer, "terminal"))
    	       openFilePointer = &fileManager.serialTerminalFile
          else
          openFilePointer = fileManager.Open(& stringBuffer)
        ...
        currentThread.myProcess.fileDescriptor[fDI] = openFilePointer
          endIf
        return fDI
    ...
    -----------------------------  Handle_Sys_Read  ---------------------------------
      ...
      if (fileDesc >  MAX_FILES_PER_PROCESS-1) || (currentThread.myProcess.fileDescriptor[fileDesc]== null) || (fileDesc < 0) || (sizeInBytes < 0)
        return -1
      endIf
      ...
      -- Terminal Branch:
        if (currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL)
          ...
          for j = 0 to (sizeInBytes - 1)
            virtAddr = buffer asInteger
            virtPage = virtAddr / PAGE_SIZE

            if virtPage < 0 || virtPage > (currentThread.myProcess.addrSpace.numberOfPages) - 1 || currentThread.myProcess.addrSpace.IsValid(virtPage) == false || currentThread.myProcess.addrSpace.IsWritable(virtPage) == false || currentThread.myProcess.addrSpace.IsValid(virtPage) == false
    	        return curCopied
    	      endIf
        ...
        -- Original Branch:
          ...
          nextPos = currentThread.myProcess.fileDescriptor[fileDesc].currentPos
          sizeOfFile = currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes

          while true

            curChunk = PAGE_SIZE - offset

            if nextPos + curChunk > sizeOfFile
              curChunk = sizeOfFile - nextPos
            endIf

            if curCopied + curChunk > sizeInBytes
              curChunk = sizeInBytes - curCopied
            endIf
            ...
            if virtPage < 0 || virtPage > (currentThread.myProcess.addrSpace.numberOfPages - 1) ||
              currentThread.myProcess.addrSpace.IsValid(virtPage) == false || currentThread.myProcess.addrSpace.IsWritable(virtPage) == false
                    return -1
            endIf

            offset = 0
            virtPage = virtPage + 1
            nextPos = nextPos + curChunk
            curCopied = curCopied + curChunk

            if curCopied == sizeInBytes
              break
            endIf
          ...
          while true
            ...
            destAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr (virtPage) + offset
            res = fileManager.SynchRead(currentThread.myProcess.fileDescriptor[fileDesc], destAddr, nextPos, curChunk)
          ...
          currentThread.myProcess.fileDescriptor[fileDesc].currentPos = nextPos
          return curCopied
        endIf
      endFunction
       ...
       virtAddr: int
        ...
        if currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL
          ...
          for j = 0 to (sizeInBytes-1)
            ...
            -- check for errir
            if virtPage < 0 || virtPage > (currentThread.myProcess.addrSpace.numberOfPages - 1) || currentThread.myProcess.addrSpace.IsWritable(virtPage) == false || currentThread.myProcess.addrSpace.IsValid(virtPage) == false
              return -1
            endIf

            characterBuffer = characterBuffer + 1
          endFor
        ...
        -- Original Branch:
          ...
          while true
            ...
            destAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr (virtPage) + offset
            res = fileManager.SynchWrite(currentThread.myProcess.fileDescriptor[fileDesc], destAddr, nextPos, curChunk)
    ...
    -----------------------------  Handle_Sys_Seek  ---------------------------------
      ...
      function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
            ...
            || newCurrentPos < -1 || newCurrentPos > currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes
              fileManager.fileManagerLock.Unlock()
              return -1
            endIf

            if newCurrentPos == -1
              currentThread.myProcess.fileDescriptor[fileDesc].currentPos = currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes
              fileManager.fileManagerLock.Unlock()
              return currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes
            endIf

            currentThread.myProcess.fileDescriptor[fileDesc].currentPos = newCurrentPos
  ...
  behavior DiskDriver
    ...
    -- busy-waiting until the disk operation has completed.
      ...
      method SynchReadSector  (sectorAddr, numberOfSectors, memoryAddr: int)
        ...
        -- It waits until the j/O is complete before returning.
        --
        ...
        --
          ...
          while true
            ...
            -- res the return status
            switch * DISK_STATUS_WORD_ADDRESS
              case DISK_STATUS_OPERATION_COMPLETED_OK:
                diskBusy.Unlock ()
                return
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_1:
                FatalError ("Disk j/O error in SynchReadSector: Memory destAddr is not page-aligned or sector count is not positive")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_2:
                FatalError ("Disk j/O error in SynchReadSector: Attempt to access invalid memory address")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_3:
                FatalError ("Disk j/O error in SynchReadSector: Bad sectorAddr or sectorCount specifies non-existant sector")
              ...
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_5:
                FatalError ("Disk j/O error in SynchReadSector: Bad command word")
              default:
                FatalError ("SynchReadSector: Unexpected status code")
            endSwitch
            -- print ("\n\nIn SynchReadSector: A simulated disk j/O error occurred...\n\n")
          endWhile
      ...
      method StartReadSector  (sectorAddr, numberOfSectors, memoryAddr: int,
        ...
        -- j/O operation is complete; if null no thread will be notified.
        --
      ...
      method SynchWriteSector  (sectorAddr, numberOfSectors, memoryAddr: int)
        ...
        -- to the disk.  It waits until the j/O is complete before returning.
        --
        -- If there is a (simulated) disk hardware failure, then this routine
        -- simply tries again in an infinite loop, until it succeeds.
        --
          ...
          while true
            ...
            switch * DISK_STATUS_WORD_ADDRESS
              case DISK_STATUS_OPERATION_COMPLETED_OK:
                ...
                return
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_1:
                FatalError ("Disk j/O error in SynchWriteSector: Memory destAddr is not page-aligned or sector count is not positive")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_2:
                FatalError ("Disk j/O error in SynchWriteSector: Attempt to access invalid memory address")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_3:
                FatalError ("Disk j/O error in SynchWriteSector: Bad sectorAddr or sectorCount specifies non-existant sector")
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_4:
                -- This case occurs when there is a hard or soft (simulated)
                -- hardware error while performing the disk operation.
                break
              case DISK_STATUS_OPERATION_COMPLETED_WITH_ERROR_5:
                FatalError ("Disk j/O error in SynchWriteSector: Bad command word")
              default:
                FatalError ("SynchWriteSector: Unexpected status code")
            endSwitch
            -- print ("\n\nIn SynchWriteSector: A simulated disk j/O error occurred...\n\n")
          endWhile
      ...
      method StartWriteSector  (sectorAddr, numberOfSectors, memoryAddr: int,
        ...
        --
        -- This method writes "numberOfSectors" sectors (of PAGE_SIZE bytes each)
        -- to the disk.  It returns immediately after starting the j/O.
        --
        -- The "whoCares" argument is a Semaphore that we will signal after the
        -- j/O operation is complete; if null no thread will be notified.
        --
          ...
          * DISK_SECTOR_NUMBER_REGISTER = sectorAddr
          * DISK_SECTOR_COUNT_REGISTER = numberOfSectors
          * DISK_COMMAND_WORD_ADDRESS = DISK_WRITE_COMMAND    -- Starts the j/O
        endMethod
  ...
  behavior FileManager
      ...
      method Init ()
        ...
        var j: int
          ...
          for j = 0 to MAX_NUMBER_OF_FILE_CONTROL_BLOCKS-1
            fcbTable[j].fcbID = j
            fcbTable[j].Init()
            fcbFreeList.AddToEnd (&fcbTable[j])
          endFor
          ...
          for j = 0 to MAX_NUMBER_OF_OPEN_FILES-1
            openFileTable[j].kind = FILE
            openFileFreeList.AddToEnd (&openFileTable[j])
          endFor
      ...
      method Print ()
        var j: int
          ...
          for j = 0 to MAX_NUMBER_OF_OPEN_FILES-1
            ...
            printInt (j)
            print (":  0x")
            printHex ((& openFileTable[j]) asInteger)
            print (":  ")
            openFileTable[j].Print()
          endFor
      ...
      -- This method is reentrant, and may block the caller.
      --
          var j, start, numFiles, fileLen, fileNameLen: int
              fcb: ptr to FileControlBlock
              p: ptr to int
          ...
          -- res the magic number
          j = *p
          p = p + 4
          if j != 0x73747562       -- in ASCII this is "stub"
          ...
          numFiles = *p
          p = p + 4
          j = *p     -- This is the nextFreeSector; ignore it.
          p = p + 4
      ...
      -- write (j.e., the buffer is dirty) then it is written out first.
      --
      ...
      method SynchRead (open: ptr to OpenFile,
          ...
          while numBytes > 0
            ...
            -- printHexVar ("         - source destAddr (posInBuffer)", posInBuffer)
...
-----------------------------  FileControlBlock  ---------------------------------
  ...
  behavior FileControlBlock
      ...
      -- in memory which may be needed when j/O is done on the file.
      --
      method Init ()
          numberOfUsers = 0
...
-----------------------------  OpenFile  ---------------------------------
  ...
  behavior OpenFile
      ...
      method ReadInt () returns int
          ...
          --
          var j: int
          if ! self.ReadBytes ((&j) asInteger, 4)
      ...
      method LoadExecutable (addrSpace: ptr to AddrSpace) returns int
        ...
        --
          var nextVirtPage, destAddr: int
              textSize, dataSize, bssSize, textStart, dataStart, bssStart: int
              j, textSizeInPages, dataSizeInPages, bssSizeInPages: int
          ...
          j = textSizeInPages + dataSizeInPages + bssSizeInPages +
          ...
          *****/
          ...
          for j = 1 to textSizeInPages
            destAddr = addrSpace.ExtractFrameAddr (nextVirtPage)
            -- printIntVar ("About to read; nextVirtPage", nextVirtPage)
            -- printHexVar ("               destAddr", destAddr)
            if ! self.ReadBytes (destAddr, PAGE_SIZE)
