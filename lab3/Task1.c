...
-----------------------------  Dining Philosophers  ----------------------------
  ...
  behavior ForkMonitor
	...
	var
	  i: int
      -- initialize such that all philosophers are thinking
	status = new array of int {5 of THINKING}
	selfCond = new array of Condition {5 of new Condition}
	lock = new Mutex
	lock.Init()
	for i = 0 to 4
	  selfCond[i].Init()	
    ...
    method Test (p: int)
	while ((status[(p + 4) % 5] != EATING) && (status[p] == HUNGRY) && (status[(p+1) % 5] != EATING))
	  status[p] = EATING
	  lock.Lock()
	  self.PrintAllStatus()
	  selfCond[p].Signal(&lock)
	  lock.Unlock()
	endWhile
    endMethod
